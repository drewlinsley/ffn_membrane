...
from utils.hybrid_utils3 import rdirs
from utils.hybrid_utils3 import pad_zeros
from utils.hybrid_utils3 import recursive_make_dir
    ...
    for x in range(path_extent[0]):
        for y in range(path_extent[1]):
            for z in range(max_z):
                ...
                # np.save(path, seg)
                it_corner = np.asarray(corner) + np.asarray([x * config.shape[0], y * config.shape[1], z * config.shape[2]])
                dataset.write((), seg.astype(np.uint32))
