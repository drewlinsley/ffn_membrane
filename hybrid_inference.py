...
def get_segmentation(
    ...
    if idx == 0:
        ...
        if membrane_slice is not None:
            assert isinstance(membrane_slice, list), 'Make membrane_slice a list.'
            # Split up membrane along z-axis into k-voxel chunks
            z_splits = np.arange(0, model_shape[0], membrane_slice[0])
            y_splits = np.arange(0, model_shape[1], membrane_slice[1])
            x_splits = np.arange(0, model_shape[2], membrane_slice[2])
            vols = []
            for z_idx in z_splits:
                for y_idx in y_splits:
                    for x_idx in x_splits:
                        vols += [vol[
                            z_idx: z_idx + membrane_slice[0],
                            y_idx: y_idx + membrane_slice[1],
                            x_idx: x_idx + membrane_slice[2]]]
                ...
                for y_idx in y_splits:
                    for x_idx in x_splits:
                        rmembranes[
                            z_idx: z_idx + membrane_slice[0],
                            y_idx: y_idx + membrane_slice[1],
                            x_idx: x_idx + membrane_slice[2]] = membranes[count]
                        count += 1
