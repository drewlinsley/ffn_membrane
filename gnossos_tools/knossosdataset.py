...
try:
    ...
    from gnossos_tools import mergelist_tools_fallback as mergelist_tools
...
class KnossosDataset(object):
    ...
    def _save(self, data, data_mag, offset, mags, as_raw, kzip_path, upsample, downsample, fast_resampling):
        ...
        for mag in mags:
            ...
            if fast and all(mag_ratio.is_integer() for mag_ratio in ratio):
                data_inter = np.array(data[::int(ratio[2]), ::int(ratio[1]), ::int(ratio[0])], dtype=datatype)
                ...
                for value in np.unique(input_chunk):
    ...
    def compress_kzip(self, kzip_path):
        ...
        with zipfile.ZipFile(kzip_path + '.k.zip', 'w', zipfile.ZIP_DEFLATED, compresslevel=1) as zf:
