...
all_segment_coords = db.pull_merge_membrane_coors()
if os.path.exists('progress/finished_merge_segs.npy'):
    finished_segments = np.load('progress/finished_merge_segs.npy')
else:
    finished_segments = glob(
        '/media/data_cifs/connectomics/ding_segmentations_merge/**/**/**/v0/0/0/seg-0_0_0.npz')
    np.save('progress/finished_merge_segs.npy', finished_segments)
...
if os.path.exists('progress/intermediate_merge_segs.npy'):
    db_paths = np.load('progress/intermediate_merge_segs.npy')
else:
    for p in tqdm(finished_paths, total=len(finished_paths)):
        for il, d in enumerate(db_paths):
            if np.all(p == d[:-1]):
                ...
                break
    np.save('progress/intermediate_merge_segs.npy', db_paths)
...
for row in tqdm(unique_paths, total=len(unique_paths)):
    path = '/media/data_cifs/connectomics/ding_segmentations_merge/x{}/y{}/z{}/v0/0/0/seg-0_0_0.npz'.format(  # noqa
        pad_zeros(row[0], 4),
        pad_zeros(row[1], 4),
        pad_zeros(row[2], 4))
...
print(fixes)
db.process_merge_segmentation_rows(fixes)
