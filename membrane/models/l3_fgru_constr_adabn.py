...
def synapse_experiment_params(
    ...
    exp = {
        ...
        'optimizer': ['nadam'],
        'training_routine': ['seung'],
        'train_input_shape': [24, 72, 72, 2],
        'train_label_shape': [36, 128, 128, 2],
...
def build_model(data_tensor, reuse, training, output_channels, merge="none"):
    ...
    with tf.variable_scope('cnn_readout', reuse=reuse):
        ...
        with tf.variable_scope('out_embedding', reuse=reuse):
            if merge == "features":
                nh2 = tf.concat([nh2, tf.expand_dims(data_tensor[..., 1], -1)], -1)  # Concat the membranes
                out_emb = conv.conv3d_layer(
                    ...
                    trainable=training,
                    use_bias=True)
                out_emb = tf.nn.elu(out_emb)
            elif merge == "attention":
                att = conv.conv3d_layer(
                    bottom=tf.expand_dims(data_tensor[..., 1], -1),
                    name='out_emb_mem',
                    stride=[1, 1, 1],
                    padding='SAME',
                    num_filters=filters[0],
