...
def get_segmentation(
    ...
    mpath = '/localscratch/middle_cube_membranes'
    membrane_slice = [64, 384, 384]
    if idx == 0:
        # 1. select a volume
        vol = np.load("/localscratch/WY05B-sv1/middle_cube.npy")
        template = np.load("template_img.npy")
        from skimage import exposure
        # Need extra normalizations for this 0-shot
        vol = vol.transpose((2, 1, 0))
        template = template.transpose((2, 1, 0))
        # normalizer = staintools.StainNormalizer(method='vahadane')
        # normalizer.fit(template[..., 128])  # Fit one slice
        # vol = normalizer.transform(vol)
        vol = exposure.match_histograms(vol, template, multichannel=True)
        vol = vol.transpose((2, 1, 0))
